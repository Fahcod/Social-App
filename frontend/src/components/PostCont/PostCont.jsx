  ...
  return (
   ...
   {posts.map((item,index)=>{
    if(item.post_type === 'image'){
      return <ImagePost key={index} post_type={item.post_type} _id={item._id} views={item.views} comments={item.comments} likes={item.likes} owner={item.owner} text={item.text} post_value={item.post_value}/>
    }else{
